...
Usage :
 ...
 > python kangaroo.py -p 02ca298581993a21f38083092001dfa86a2f4aa06e75867a3fece2b5b96a253e20 -keyspace 1:ffffffffffff -ntu -dispset 1
...
parser.add_argument("-dispset", help = "Display Setting for show progress. default=0", action='store')
# 1=Scan speed.   2=Speed+Count.  3=Speed+Time.  4=Speed+Count+Time.  0=Show All  
...
dispset = int(args.dispset) if args.dispset else 0
...
ice.run_cpu_kangaroo.argtypes = [ctypes.c_bool, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int] # ntu,st,en,dp,ncpu,mx,pvk,upub,disp
ice.get_reduced_upub.argtypes = [ctypes.c_char_p, ctypes.c_char_p, ctypes.c_char_p] # stkey, upub, res
ice.init_kangaroo_lib()

###############################################################################
def run_cpu_kangaroo(tame_update_file, start_range_int, end_range_int, dp, ncpu, mx, upub_bytes, dispset):
    st_hex = hex(start_range_int)[2:].encode('utf8')
    en_hex = hex(end_range_int)[2:].encode('utf8')
    res = (b'\x00') * 32
    ice.run_cpu_kangaroo(tame_update_file, st_hex, en_hex, dp, ncpu, mx, res, upub_bytes, dispset)
    return res
...
#    rpub = reduced_key(range_st, upub)
    ...
    if int(rpub.hex()[2:], 16) == 0: 
        ...
        break
    pvk_found = run_cpu_kangaroo(tame_update_file, 0, range_en-range_st, dp, ncore, mx, rpub, dispset)
